...
BASIS_BASH_UTILITIES=\"$__DIR__/${BASH_LIBRARY_DIR}/${PREFIX}basis.sh\""
      ...
      set (BASHPATH)
      foreach (DIR ${ARGN_LINK_DEPENDS})
        message ("** basis_configure_script(): DIR 1: ${DIR}")
        if (DIR MATCHES "^relative +(.*)$")
          basis_get_relative_path (DIR "${__DIR__}" "${CMAKE_MATCH_1}")
        endif ()
        message ("** basis_configure_script(): DIR 2: ${DIR}")
        if (DIR MATCHES "\\.sh$")
          get_filename_component (DIR "${DIR}" PATH)
        endif ()
        message ("** basis_configure_script(): DIR 3: ${DIR}")
        if (IS_ABSOLUTE "${DIR}")
          message ("** basis_configure_script(): DIR 4: ${DIR}")
          list (APPEND BASHPATH "${DIR}")
        else ()
          message ("** basis_configure_script(): DIR 4: $__DIR__/${DIR}")
