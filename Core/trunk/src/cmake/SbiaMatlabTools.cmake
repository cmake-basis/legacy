...
if (NOT SBIA_MATLABTOOLS_INCLUDED)
set (SBIA_MATLABTOOLS_INCLUDED 1)
...
find_file (
  SBIA_SCRIPT_RUN_MCC
    NAMES SbiaRunMcc.m
    HINTS "${SBIA_CMakeModules_MODULE_PATH}"
...
# directory that was added via sbia_include_directories ().

function (sbia_create_addpaths_mfile)
  set (MFILE "${CMAKE_CURRENT_BINARY_DIR}/Add${PROJECT_NAME}Paths.m")

  file (WRITE "${MFILE}" "% DO NOT edit. This file is automatically generated by SBIA CMake functions.\n")
...
# using the MATLAB Compilier (MCC). It is invoked by sbia_add_executable ()
# and sbia_add_library (), respectively, when at least one M-file is given
...
#       by either sbia_project_finalize () or sbia_superproject_finalize ().
...
function (sbia_add_mcc_target TARGET_NAME)
  ...
  # set target properties required by sbia_add_mcc_target_finalize ()
  set_target_properties (
    ...
    PROPERTIES
      ...
      INCLUDE_DIRECTORIES       "${SBIA_INCLUDE_DIRECTORIES}"
...
# added by sbia_add_mcc_target () to create the custom build command and
# adds this build command as dependency of this added target.
#
# \see sbia_add_mcc_target ()
#
# \param [in] TARGET_UID "Global" target name. If this function is used
#                        within the same project as sbia_add_mcc_target (),
#                        the "local" target name may be given alternatively.

function (sbia_add_mcc_target_finalize TARGET_UID)
  ...
  if (NOT SBIA_TYPE MATCHES "^MCC_EXECUTABLE$|^MCC_LIBRARY$")
    message (FATAL_ERROR "Target ${TARGET_UID} had invalid SBIA_TYPE: ${SBIA_TYPE}")
  endif ()
  ...
  foreach (INCLUDE_PATH ${INCLUDE_DIRECTORIES})      # add directories added via
    list (FIND MCC_ARGS "${INCLUDE_PATH}" IDX)       # sbia_include_directories ()
  ...
  if (MCC_MATLAB_MODE)
    ...
    if (NOT SBIA_CMD_MATLAB)
      message ("MATLAB not found. It is required to build target ${TARGET_UID} in MATLAB mode."
    ...
    if (NOT SBIA_SCRIPT_RUN_MCC)
      message ("MATLAB script to run MATLAB Compiler not found. It is required to build target ${TARGET_UID} in MATLAB mode."
  ...
  if (TYPE STREQUAL "LIBRARY")
    set (BUILD_OUTPUT "${CMAKE_LIBRARY_OUTPUT_DIRECTORY}/${OUTPUT_NAME}")
    ...
    set (BUILD_OUTPUT "${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/${OUTPUT_NAME}")

    set (
      POST_BUILD_COMMAND
        COMMAND "${CMAKE_COMMAND}" -E copy
                "${BUILD_DIR}/${OUTPUT_NAME}"
                "${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/${OUTPUT_NAME}"
    )
...
endif (NOT SBIA_MATLABTOOLS_INCLUDED)
